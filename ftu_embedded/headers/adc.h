 ...
 * adc_header.h
 *
 * Created: 2020-12-02 11:20:48 AM
 *  Author: Rohit
 */ 
...
void receive_test_instructions(void){
	// Create a variable that holds the received instructions
	String received_instruction;
	// Create a variable that holds the json instruction in string format from the GUI
	String instruction = "instruction";
	
	// Wait for something to arrive at serial
	while (Serial.available()==0){
		continue;
	}
	
	// Once we receive something, read it until we hit the end of the line
	received_instruction = Serial.readStringUntil('\n');
	
	// Wait for the instruction JSON at serial
	while (Serial.available()==0){
		// If the received instruction = instruction, we are about to receive the instructions
		if (received_instruction.equals(instruction)){
			Serial.println("ready");
		}
	}
	
	// Update the received_instruction variable to contain the received instruciton object
	received_instruction = Serial.readStringUntil('\n');
	
	// Print the received_instruction for clarity
	Serial.println(received_instruction);
	...
	// Parse the JSON object to retrieve data
	test_id							= doc["id"];						// 0
	const char* description			= doc["description"];				// "Apply required stress for 2 hours, etcetera"
	JsonObject test_values			= doc["test_values"];				// Get the test parameters and store in it's own JSON
	JsonObject measurement_params	= doc["measurement_params"];		// Get the measurement params and store in it's own JSON
	
	// Parse through our test_values JSON object
	...
	// Parse through our measurement_params JSON object

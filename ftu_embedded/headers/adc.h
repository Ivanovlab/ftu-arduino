...
#define ADC_VREF 4.4 //3.3V //this is VREFP-VREFN

int twos_complement_to_int (int value){
	if (number_of_bits_adc <= 2 || number_of_bits_adc == '\0' || value > (pow(2,number_of_bits_adc)-1)) return '\0'; //error checking
	else if (value >> (number_of_bits_adc-1)) return (( value & (int)(pow(2,number_of_bits_adc-1)-1)) - (pow(2,number_of_bits_adc-1))); //negative values
...
void adc_fixed_scan(double adc_fixed_array[][100], int samples_per_channel){
	uint8_t status_byte;
	uint16_t value;
	SPI.begin();
	...
	for (int i=0; i<8; i++){ //the "high sample rate (HSR)" array to be sent to serial port filled once/serial_rate
		...
		for(int j=0; j<samples_per_channel; j++){
			...
			delayMicroseconds(8); //delay based off data rate selected
		}
	}
...
void adc_auto_scan(double adc_auto_array[][21], int samples_per_channel){
	uint8_t status_byte;
	uint16_t value;
	SPI.begin();
	SPI.beginTransaction(SPISettings(adc_spi_speed,MSBFIRST,SPI_MODE0));
	...
	SPI.transfer(0b00111101); //enable all system readings (SYSRED)
	adc_toggle_cs();
	for (int j=0; j<samples_per_channel; j++){
		for (int i=0; i<21; i++){
			...
			status_byte = SPI.transfer(0);
			value = SPI.transfer16(0);
			adc_auto_array[j][i] = (double)(twos_complement_to_int(value)*ADC_VREF)/(30.72*ADC_GAIN);
		}
	}
	SPI.endTransaction();
}
